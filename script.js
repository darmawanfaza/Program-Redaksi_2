...
const addBtnDefaultText = addBtn ? addBtn.textContent : "";
let editIndex = null;
...
function addEntryFromForm() {
  ...
  if (barang === "lainnya" && !barangCustom) {
    setError(document.getElementById("barangCustom"), null, "Isi jenis barang untuk opsi 'Lainnya'.");
    alert("Untuk opsi 'Lainnya', isi jenis barang pada kolom yang tersedia.");
    return;
  }
  if (berat <= 0) {
    setError(document.getElementById("berat"), null, "Berat harus lebih besar dari 0.");
    alert("Berat harus lebih besar dari 0.");
    return;
  }
  if (barang === "emas batangan" && (!merkEmas || !noSeriEmas)) {
    setError(document.getElementById("merkEmas"), null, "Merk dan nomor seri wajib diisi.");
  ...
  if (adaBatu && stones.length === 0 && barang !== "batu") {
    setError(document.getElementById("batu1_jml"), null, "Isi jumlah dan jenis batu jika checkbox batu dicentang.");
    alert("Checkbox batu dicentang, tetapi belum ada data batu yang diisi.");
    return;
  }
  if (harga === 0 && isEmas) {
    const hargaErrorEl = document.getElementById("hargaError");
    setError(document.getElementById("harga"), hargaErrorEl,
      "Emas dengan berat > 0 dan kadar valid biasanya diberi nilai taksiran. Yakin 0?");
    // tidak wajib return, hanya warning visual
  }
...
if (barangSelect && rowBarangCustom) {
  barangSelect.addEventListener("change", () => {
    if (barangSelect.value === "lainnya") {
      ...
      if (barangCustomInput) barangCustomInput.value = "";
    }
  });
}
